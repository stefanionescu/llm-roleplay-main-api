...
for server in "${SERVERS[@]}"; do
  ...
  ssh -o ConnectTimeout=10 -o StrictHostKeyChecking=no root@$server "cd /opt/llm-roleplay-main-api && docker ps -q | xargs -I {} docker exec {} ls -la /app/dist" || echo "Warning: Failed to connect or run command on $server"
done

echo "Dist check complete."